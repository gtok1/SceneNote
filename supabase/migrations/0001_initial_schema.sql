...
CREATE EXTENSION IF NOT EXISTS "pgcrypto"; -- gen_random_uuid() 방어용
...
CREATE TRIGGER trg_content_external_ids_updated_at
  BEFORE UPDATE ON content_external_ids
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE TRIGGER trg_user_episode_progress_updated_at
  BEFORE UPDATE ON user_episode_progress
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
-- timeline_pin_tags
CREATE TRIGGER trg_timeline_pin_tags_updated_at
  BEFORE UPDATE ON timeline_pin_tags
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

-- external_search_cache
CREATE TRIGGER trg_external_search_cache_updated_at
  BEFORE UPDATE ON external_search_cache
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

-- metadata_sync_logs
CREATE TRIGGER trg_metadata_sync_logs_updated_at
  BEFORE UPDATE ON metadata_sync_logs
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE POLICY "timeline_pin_tags_select_own" ON timeline_pin_tags
  ...
  USING (
    ...
    AND EXISTS (
      SELECT 1 FROM tags
      WHERE tags.id = timeline_pin_tags.tag_id
...
-- IDX-04B: 콘텐츠/에피소드 조합 기준 핀 조회 — 관리자/집계/향후 공개 핀 확장용
CREATE INDEX idx_timeline_pins_content_episode_ts
  ON timeline_pins (content_id, episode_id, timestamp_seconds ASC NULLS LAST);
